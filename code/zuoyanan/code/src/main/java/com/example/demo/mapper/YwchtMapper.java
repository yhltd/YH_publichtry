...
@Mapper
@Repository
public interface YwchtMapper extends BaseMapper<Htjl> {
    //显示全部
    @Select("SELECT * FROM hetong_jilu WHERE F IN ('未对账', '已对账')")
    List<Htjl> getList();
    ...
    @Update({
            ...
            "END,",
            "V = CASE",
            "    WHEN V IS NULL OR V = 0 THEN U * 40",
            "    ELSE V",
            "END",
            "X = CASE",
    ...
    //设置对账
    @Update({"UPDATE hetong_jilu set f = '已对账' where id=#{id} "})
    boolean szdz(int id);
    //设置开票
    @Update({"UPDATE hetong_jilu set f = '未开票' where id=#{id} "})
    boolean szkp(int id);
}
