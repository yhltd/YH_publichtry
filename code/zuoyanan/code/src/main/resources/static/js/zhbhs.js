...
function getList() {
    ...
    $ajax({
        ...
        if (res.code == 200) {
            setTable(res.data);
            // colResizable是jquery表格拖拽插件
            $("#zhbhsTable").colResizable({
                liveDrag: true,
...
function setTable(data) {
    if ($('#zhbhsTable').html != '') {
        $('#zhbhsTable').bootstrapTable('load', data);
    }

    $('#zhbhsTable').bootstrapTable({
        data: data,
        sortStable: true,
        ...
        columns: [
            {
                ...
                width: 130,
                formatter: function (value, row, index) {
                    if(row.je != '' && row.je != NaN && row.sd != '' && row.sd != 0){
                        var bhsje = row.je / row.sd;
                        return bhsje.toFixed(2)
                    }
                }
            }
        ],
