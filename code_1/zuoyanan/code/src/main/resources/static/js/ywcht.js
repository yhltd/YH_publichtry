...
$(function () {
    ...
    $('#update-btn').click(function () {
        let rows = getTableSelection('#ywchtTable');
    ...
    $('#szdz-btn').click(function () {
        var msg = confirm("确认要将选中数据设置对账吗？");
        if (msg) {
            let rows = getTableSelection("#ywchtTable");
            if (rows.length == 0) {
                swal('请选择要对账的数据！');
    ...
    $('#szkp-btn').click(function () {
        var msg = confirm("确认要将选中数据移动到开票吗？");
        if (msg) {
            let rows = getTableSelection("#ywchtTable");
            if (rows.length == 0) {
                swal('请选择要开票的数据！');
...
function setTable(data) {
    if ($('#ywchtTable').html != '') {
        $('#ywchtTable').bootstrapTable('load', data);
    }
    $('#ywchtTable').bootstrapTable({
        data: data,
        sortStable: true,
